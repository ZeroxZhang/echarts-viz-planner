...
`infographic.process/swimlane/mechanism/journey/capability/condition/dependency` 与原 `infographic` 一样使用 `kind: infographic` 和开放 `structure`；未修改 1.0/1.1 的必需字段及默认行为。新增哑铃与坡度入口的映射仍是 `kind: echarts`，custom 函数仍由载体实现。目录、运行条件和退化策略从独立源维护；通用图示交接见 `diagram-handoff.md`。
